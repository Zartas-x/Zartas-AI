...
class ZartasAIApp(App):
    def build(self):
        ...
        self.chat_log = Label(text="[color=00FF00][Zartas AI]:[/color] Система стабилизирована. Проверяю связь...\n", 
    ...
    def fetch_ai_response(self, text):
        try:
            # Используем актуальный URL для Gemini 1.5 Flash
            ...
            if r.status_code == 200:
                ...
                self.chat_log.text += f"\n[Ошибка API {r.status_code}]: Попробуй включить VPN.\n"
        except Exception as e:
            self.chat_log.text += f"\n[Ошибка сети]: {str(e)[:50]}\n"

    def self_improve(self, instance):
        self.chat_log.text += "\n[AI]: Создаю тестовый файл эволюции на GitHub...\n"
        # Тестовый контент вместо чтения живого кода (чтобы не вылетало)
        test_content = "Evolution test success. AI can write to repo."
        Clock.schedule_once(lambda dt: self.push_to_github("evolution_log.txt", test_content), 0.5)
    ...
    def push_to_github(self, file_path, content):
        try:
            ...
            # Проверка наличия файла для получения SHA
            ...
            if res.status_code in [200, 201]:
                self.chat_log.text += f"\n[УСПЕХ]: Файл {file_path} создан! Токен работает.\n"
            else:
                self.chat_log.text += f"\n[GitHub Error {res.status_code}]: Проверь права токена.\n"
        except Exception as e:
            self.chat_log.text += f"\n[Ошибка]: {str(e)}\n"
